...
testResult minion_choice_2_adds_4_cards(struct gameState gameState)
{
  noisyprint("Minion choice 2 adds 4 cards:\n");
  struct gameState testState;
  memcpy(&testState, &gameState, sizeof(struct gameState));
  memset(testTitle, '\0', 256);
  if(!NOISY_TEST) strncpy(testTitle, "Minion choice 2 adds 4 cards.", 255);
